...
"[project]/pages/api/auth/update-profile.ts [api] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function handler(req, res) {
    ...
    try {
        ...
        is_client, joined_at, renewal_at, expire_at } = req.body;
        ...
        const { error: updateError } = await clientToUse.from('user_profiles').update(updateData).eq('user_id', authUser.id);
        if (updateError) {
        ...
        if (user_name !== undefined || contact_no !== undefined) {
            const { error: metadataError } = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabase$2e$ts__$5b$api$5d$__$28$ecmascript$29$__["supabase"].auth.updateUser({
                data: {
                    display_name: user_name || authUser.user_metadata?.display_name || null,
                    user_name: user_name || authUser.user_metadata?.user_name || null,
                    phone: contact_no || authUser.user_metadata?.phone || null,
                    contact_no: contact_no || authUser.user_metadata?.contact_no || null
                }
            });
            if (metadataError) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__dd29c65c._.js.map