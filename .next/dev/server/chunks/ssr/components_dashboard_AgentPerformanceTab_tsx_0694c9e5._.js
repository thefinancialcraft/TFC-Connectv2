...
function AgentPerformanceTab({ selectedOrgId, selectedUserId, dateFilter: propDateFilter = "today", loading = false }) {
    ...
    const fetchRpcPerformance = async (start, end)=>{
        try {
            setRpcLoading(true);
            const { data, error } = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabase$2e$ts__$5b$ssr$5d$__$28$ecmascript$29$__["supabase"].rpc('get_org_performance_report', {
                p_start_date: start,
                p_end_date: end
            });
