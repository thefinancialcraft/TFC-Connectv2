...
function useActivityData() {
    ...
    const fetchActivities = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$react$2f$index$2e$js__$5b$client$5d$__$28$ecmascript$29$__["useCallback"])({
        ...
        `).gte("created_at", startOfDay).lte("created_at", endOfDay).order("created_at", {
                ...
                const getLevel = {
                    "useActivityData.useCallback[fetchActivities].getLevel": ()=>{
                        const designation = (user.designation || '').toLowerCase();
                        const role = user.role || '';
                        if (user.isClient === false && (role === 'superadmin' || role === 'super_admin') && (designation === 'ceo' || designation === 'developer')) return 1; // LEVEL_1_ADMIN
                ...
                if (dashboardLevel === 4) {
                    ...
                    if (teamData) {
                        teamData.forEach({
                            "useActivityData.useCallback[fetchActivities]": (t)=>{
                                if (Array.isArray(t.members)) t.members.forEach({
                                    "useActivityData.useCallback[fetchActivities]": (m)=>teamMemberIds.push(String(m))
                                }["useActivityData.useCallback[fetchActivities]"]);
                            }
                        }["useActivityData.useCallback[fetchActivities]"]);
                    }
    ...
    const fetchMobileHistory = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$react$2f$index$2e$js__$5b$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "useActivityData.useCallback[fetchMobileHistory]": async (isBackground = false)=>{
            ...
            try {
                ...
                const getLevel = {
                    "useActivityData.useCallback[fetchMobileHistory].getLevel": ()=>{
                        const designation = (user.designation || '').toLowerCase();
                        const role = user.role || '';
                        if (user.isClient === false && (role === 'superadmin' || role === 'super_admin') && (designation === 'ceo' || designation === 'developer')) return 1;
                ...
                if (dashboardLevel === 4) {
                    ...
                    if (teamData) {
                        teamData.forEach({
                            "useActivityData.useCallback[fetchMobileHistory]": (t)=>{
                                if (Array.isArray(t.members)) t.members.forEach({
                                    "useActivityData.useCallback[fetchMobileHistory]": (m)=>teamMemberIds.push(String(m))
                    ...
                    const { data: profiles } = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabase$2e$ts__$5b$client$5d$__$28$ecmascript$29$__["supabase"].from('user_profiles').select('employee_id').in('user_id', uniqueUserIds);
                    const empIds = (profiles || []).map({
                        "useActivityData.useCallback[fetchMobileHistory].empIds": (p)=>p.employee_id
        ...
        "useActivityData.useEffect": ()=>{
            const fetchGlobalFilters = {
                "useActivityData.useEffect.fetchGlobalFilters": async ()=>{
                    if (!mounted || !user) return;
                    const getLevel = {
                        "useActivityData.useEffect.fetchGlobalFilters.getLevel": ()=>{
                            const designation = (user.designation || '').toLowerCase();
                    ...
                    // 2. All Campaigns
                    let campQuery = __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabase$2e$ts__$5b$client$5d$__$28$ecmascript$29$__["supabase"].from('campaigns').select('id, name').order('name');
                    if (dashboardLevel !== 1 && user.organization_id) {
                    ...
                    // 3. All Agents
                    let agentQuery = __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabase$2e$ts__$5b$client$5d$__$28$ecmascript$29$__["supabase"].from('user_profiles').select('employee_id, user_name, user_id').order('user_name');
                    if (dashboardLevel === 4) {
                        ...
                        if (teamData) {
                            teamData.forEach({
                                "useActivityData.useEffect.fetchGlobalFilters": (t)=>{
                                    if (Array.isArray(t.members)) t.members.forEach({
                                        "useActivityData.useEffect.fetchGlobalFilters": (m)=>teamMemberIds.push(String(m))
                                    }["useActivityData.useEffect.fetchGlobalFilters"]);
                                }
                            }["useActivityData.useEffect.fetchGlobalFilters"]);
                        }
                        agentQuery = agentQuery.in('user_id', [
                            ...new Set(teamMemberIds)
                        ]);
                    } else if (dashboardLevel === 2) {
                    ...
                    let logQuery = __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabase$2e$ts__$5b$client$5d$__$28$ecmascript$29$__["supabase"].from('call_logs').select('disposition').not('disposition', 'is', null);
                    if (dashboardLevel !== 1 && user.organization_id) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__397ada21._.js.map