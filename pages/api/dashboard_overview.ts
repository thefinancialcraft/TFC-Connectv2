...
export default async function handler(
  ...
  try {
    ...
    const fetchPromises = [
      ...
      (async () => {
        let q = dbClient.from("call_history").select("*", { count: "exact", head: true }).gte("timestamp", range.start).lte("timestamp", range.end);
      ...
      (async () => {
        return fetchSumData(dbClient, "call_history", "duration", {
          startDate: range.start, endDate: range.end, dateColumn: "timestamp",
          employeeId: filterEmployeeId || restrictedEmployeeIds
        });
      })(),
      // 6. Total Connections (Count Only)
      (async () => {
        let q = dbClient.from("call_history").select("*", { count: "exact", head: true }).gte("timestamp", range.start).lte("timestamp", range.end).gt('duration', 0);
        if (filterEmployeeId) q = q.eq('employee_id', filterEmployeeId);
        else if (restrictedEmployeeIds && restrictedEmployeeIds.length > 0) q = q.in('employee_id', restrictedEmployeeIds);
        const { count } = await q; return count || 0;
      })(),
      ...
      (async () => {
        const [fresh, totalRecs, followups, overdue] = await Promise.all([
            // Fresh Global
            (async () => {
                let q = dbClient.from("customers").select("*", { count: "exact", head: true }).is('disposition', null).eq('attempt_count', 0);
                if (targetOrgId) q = q.eq("organization_id", targetOrgId);
                if (filterUserId && filterUserId !== 'all') q = q.eq('assigned_to', filterUserId);
                else if (restrictedUserIds && restrictedUserIds.length > 0) q = q.in('assigned_to', restrictedUserIds);
                const { count } = await q; return count || 0;
            ...
            // All-time Total
            (async () => {
                let q = dbClient.from("customers").select("*", { count: "exact", head: true });
                if (targetOrgId) q = q.eq("organization_id", targetOrgId);
                if (filterUserId && filterUserId !== 'all') q = q.eq('assigned_to', filterUserId);
                else if (restrictedUserIds && restrictedUserIds.length > 0) q = q.in('assigned_to', restrictedUserIds);
                const { count } = await q; return count || 0;
            ...
            // Followups
            (async () => {
                let q = dbClient.from("customers").select("*", { count: "exact", head: true }).in('disposition', ['Callback', 'Call Back', 'Follow Up', 'FollowUp']);
                if (targetOrgId) q = q.eq("organization_id", targetOrgId);
                if (filterUserId && filterUserId !== 'all') q = q.eq('assigned_to', filterUserId);
                else if (restrictedUserIds && restrictedUserIds.length > 0) q = q.in('assigned_to', restrictedUserIds);
                const { count } = await q; return count || 0;
            ...
            (async () => {
                ...
                let q = dbClient.from("customers").select("*", { count: "exact", head: true }).in('disposition', ['Callback', 'Call Back', 'Follow Up', 'FollowUp']).or(`next_called_at.lt.${nowIso},next_called_at.is.null`);
                if (targetOrgId) q = q.eq("organization_id", targetOrgId);
                if (filterUserId && filterUserId !== 'all') q = q.eq('assigned_to', filterUserId);
                else if (restrictedUserIds && restrictedUserIds.length > 0) q = q.in('assigned_to', restrictedUserIds);
                const { count } = await q; return count || 0;
